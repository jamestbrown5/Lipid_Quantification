...
ggplot(data=dataL1,aes(x=sample_day,y=lipid_mass,fill=strain))+
  stat_summary(aes(y = lipid_mass), fun.y=mean,geom="bar",position="dodge")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.05,position=position_dodge(0.95)) +
  scale_fill_manual(values=c(UZ="red",BE="blue"))+
...
mixed1=lmer(lipid_mass ~ strain*sample_day+lean_mass + (1|rep/cohort) ,data=dataWeDW, REML = TRUE)
step(mixed1)
