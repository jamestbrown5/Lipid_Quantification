...
```{r}
ggplot(all, aes(x=wday,fill=treat,group=treat))+
  geom_density(alpha=.2,adjust=2)+
  xlab("Days in 5th Instar")+
  theme_classic()+
  scale_x_continuous(breaks = seq(0, 17, by=1), limits=c(0,17))+
  scale_fill_manual(values=c("12"="blue","16"="green"))+
  ggtitle("Days to Wandering Stage")

modtreat=lmer(wday~treat*strain+(1|cohort),all)
plot(modtreat)
summary(modtreat)
...
mod16=lmer(wday~strain*treat+(1|cohort),all)
plot(mod16)
summary(mod16)
modaov16=aov(all$wday~all$strain)
summary(modaov16)
```
